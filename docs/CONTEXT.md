...
- **Heavy Rotation** — Live ranking of distinct tracks by `play_completed` count in the **current UTC calendar month** (“this month so far”). Cap **50**. Complements frozen **On-Repeat Snapshot**s (previous month). Surfaces: (1) a listening-hub strip (hide when empty), (2) a **Personal Library** (`/library`) sort option that reorders the library list by the same signal. Not a dated snapshot — recomputed on read.
...
63. **Heavy Rotation (live this month)** — Rank distinct tracks by `play_completed` count in the **current UTC calendar month**. Cap **50**; **hide** home strip when empty. Surfaces: listening-hub strip **and** a `/library` sort option. Complements **On-Repeat Snapshot** (frozen previous month) — Heavy Rotation is live, not persisted. See [ADR-026](./decisions/026-heavy-rotation.md).
