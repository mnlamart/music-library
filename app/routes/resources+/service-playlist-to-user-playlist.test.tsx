...
function makeRequest(formData: FormData) {
  return new Request("http://localhost/resources/service-playlist-to-user-playlist", {
    method: "POST",
    body: formData,
  });
}
...
describe("service-playlist-to-user-playlist action", () => {
  ...
  describe("validation", () => {
    ...
    test("returns 400 when action is invalid", async () => {
      vi.mocked(prisma.servicePlaylist.findFirst).mockResolvedValue(mockServicePlaylist as any);
  ...
  describe("create mode", () => {
    ...
    test("creates playlist and bulk-adds tracks in transaction", async () => {
      ...
      vi.mocked(prisma.$transaction).mockImplementation(async (fn: any) =>
        fn({
          userPlaylist: {
            create: vi.fn().mockResolvedValue({ id: "up-1", title: "Chill Vibes" }),
          },
  ...
  describe("add mode", () => {
    ...
    test("skips duplicates and reports counts", async () => {
      ...
      vi.mocked(prisma.userPlaylistTrack.createMany).mockResolvedValue({} as any);
