import { data, Form, Link, useNavigation, redirect } from 'react-router'
...
export async function action({ request, params }: Route.ActionArgs) {
	...
	if (action === 'cancel') {
		return redirect(`/library/import/${serviceName}`)
	}
...
export default function ImportFromServicePage({ loaderData, actionData }: Route.ComponentProps) {
	...
	const primaryAction = hasPreview && actionData.previewData.alreadyExists ? {
		...
		formAction: `/library/import/${loaderData.service.name}`,
		formData: { 
			action: 'import',
	...
	return (
		<div className="flex flex-col gap-6">
			...
			<p className="text-muted-foreground">
				Import tracks from {loaderData.service.displayName} by pasting a video URL. You'll be able to preview the track before adding it to your library.
			</p>
						...
						<div className="space-y-2">
							...
							<div className="flex gap-2">
								<Input
									...
									name="url"
									type="url"
									placeholder={`${loaderData.service.baseUrl}/...`}
									required
									defaultValue={''}
									className="flex-1"
								/>
							...
							<p className="text-sm text-muted-foreground">
								Paste a {loaderData.service.displayName} URL to preview and import it to your library
							</p>
						</div>
					...
					<PreviewCard
						...
						content={{
							...
							badges: [
								...
								{ label: loaderData.service.displayName, variant: 'outline' }
							]
						}}
