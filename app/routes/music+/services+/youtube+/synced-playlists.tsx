...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const { totalTracks, missingTracks } = await getSyncedPlaylistsTrackStats(userId);
    ...
    <div className="py-8">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage your synchronized YouTube playlists</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        {playlists.length > 0 && missingTracks > 0 && (
          ...
          >
            <Icon name="plus" className="h-4 w-4 mr-2" />
            {isAddingAll ? "Adding..." : `Add All Missing to Library (${missingTracks})`}
          </Button>
        )}
      </div>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <CardTitle className="text-lg line-clamp-2 mb-1">{playlist.title}</CardTitle>
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-muted-foreground mb-6">
              You haven't synced any YouTube playlists yet. Discover and sync your playlists to get
              started.
            </p>
      ...
      <AlertDialog open={isAddAllDialogOpen} onOpenChange={setIsAddAllDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Add All Missing to Library</AlertDialogTitle>
            <AlertDialogDescription>
              Add {missingTracks} track{missingTracks !== 1 ? "s" : ""} from {playlists.length}{" "}
              synced playlist
              {playlists.length !== 1 ? "s" : ""} to your library? Tracks already in your library
              will be skipped.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
