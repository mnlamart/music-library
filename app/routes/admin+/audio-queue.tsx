...
} from "#app/features/audio-archive/worker.server.ts";
...
export default function AudioQueueRoute({ loaderData }: Route.ComponentProps) {
  const [searchParams] = useSearchParams();
  const activeFilter = (searchParams.get("status") ?? "all") as StatusFilter;
  const currentPage = Number(searchParams.get("page") ?? "1");

  const { workerState, queueStats, jobs, currentlyProcessingTrack, totalPages } = loaderData;
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t">
            ...
            <div className="flex gap-1">
              <Form method="get" className="inline">
                ...
                {currentPage > 1 && <input type="hidden" name="page" value={currentPage - 1} />}
                <Button type="submit" variant="outline" size="sm" disabled={currentPage <= 1}>
