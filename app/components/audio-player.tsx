...
function PlayerTransportControls({
  ...
  const navButtonClass = size === "large" ? "h-11 w-11 p-0" : "h-8 w-8 p-0";
...
function PlayerNowPlayingSheet({
  ...
  return (
    <>
      ...
      <TrackDetailsDialog trackId={track.id} open={isDetailsOpen} onOpenChange={setIsDetailsOpen} />
    </>
  );
}
...
export function AudioPlayer(props: AudioPlayerProps) {
  ...
  useEffect(() => {
    ...
    navigator.mediaSession.metadata = new MediaMetadata(buildMediaSessionMetadata(track));
    ...
    navigator.mediaSession.setActionHandler("seekto", (details) => {
      ...
      audio.currentTime = clampMediaSessionSeekTime(details.seekTime, trackDuration);
      ...
      <div className="md:hidden">
        <div className="flex items-center gap-2 px-3 py-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-muted-foreground">Press play to start listening</p>
          </div>
          <Button
            variant="default"
            size="lg"
        ...
        <div className="flex-1 mt-6 min-h-0 flex flex-col gap-4">
          {isEmpty ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">Add tracks to your queue to see them here.</p>
            </div>
          ) : (
            <>
              {currentTrack ? (
...
function QueueTrackItem({
  ...
  return (
    <div
      className={`group flex items-center gap-3 px-4 py-3 rounded-md hover:bg-muted/50 transition-colors ${
