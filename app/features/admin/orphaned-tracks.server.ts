...
export async function getOrphanedTrackStats(
  ...
  const storageOrphans = await prisma.trackAudioFile.count({
    where: { track: null },
  });

  const orphanedFiles = await prisma.trackAudioFile.findMany({
    where: { track: null },
    select: { fileSize: true },
  });
  const storageWasteBytes = orphanedFiles.reduce(
...
export async function cleanupOrphanedAudioFiles(
  ...
  const files = await prisma.trackAudioFile.findMany({
    where: { id: { in: fileIds }, track: null },
