...
if (process.env.AUDIO_ARCHIVE_ENABLED === "true") {
  ...
  void import("./features/audio-archive/worker.server.ts").then(({ processQueueTick }) => {
    const runTick = async () => {
...
export default async function handleRequest(...args: DocRequestArgs) {
  const [request, responseStatusCode, responseHeaders, reactRouterContext, loadContext] = args;
  ...
  const nonce = getServerAppContext(loadContext)?.nonce ?? crypto.randomBytes(16).toString("hex");
